...
void Snake::setDirection(const Direction& _direction)
{
    ...
    game->setPausedState(0);    // nomal playing state
}
...
{
    out << _snake.body << _snake.direction << _snake.growing << _snake.speed << _snake.steps << _snake.timeCounter;
    return out;
}
...
{
    int _direction;
    in >> _snake.body >> _direction >> _snake.growing >> _snake.speed >> _snake.steps >> _snake.timeCounter;
